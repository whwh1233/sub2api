...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		// 发送请求
		resp, err = s.doClaudeUpstreamWithTLS(upstreamReq, proxyURL, account, "messages", attempt, tlsProfile)
		if err != nil {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := s.doClaudeUpstreamWithTLS(retryReq, proxyURL, account, "messages_signature_retry", attempt+1, tlsProfile)
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := s.doClaudeUpstreamWithTLS(retryReq2, proxyURL, account, "messages_tool_signature_retry", attempt+2, tlsProfile)
										if retryErr2 == nil {
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := s.doClaudeUpstreamWithTLS(budgetRetryReq, proxyURL, account, "messages_budget_retry", attempt+1, tlsProfile)
							if retryErr == nil {
