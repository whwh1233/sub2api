...
//
// 形态对齐官方 Claude Code CLI 2.1.241（Syi）：
//
//	x-anthropic-billing-header: cc_version=2.1.241.{fp}; cc_entrypoint=cli; cch=00000;
//
// 2.1.x 中段一度去掉 HMAC 形式的 cch=...（issue #3358）。2.1.241 在 first-party /
// vertex 路径重新带上占位字段 `cch=00000;`（不是旧的 HMAC 签名）。继续省略它会
// 让伪装流量短一截。cc_version + cc_entrypoint=cli 仍是客户端识别与第一方判定
// 的稳定信号。
//
