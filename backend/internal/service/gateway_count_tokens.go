...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	// 发送请求
	resp, err := s.doClaudeUpstreamWithTLS(upstreamReq, proxyURL, account, "count_tokens", 1, s.tlsFPProfileService.ResolveTLSProfile(account))
	if err != nil {
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
		...
		if buildErr == nil {
			retryResp, retryErr := s.doClaudeUpstreamWithTLS(retryReq, proxyURL, account, "count_tokens_signature_retry", 2, s.tlsFPProfileService.ResolveTLSProfile(account))
			if retryErr == nil {
...
func (s *GatewayService) forwardCountTokensAnthropicAPIKeyPassthrough(ctx context.Context, c *gin.Context, account *Account, body []byte) error {
	...
	resp, err := s.doClaudeUpstreamWithTLS(upstreamReq, proxyURL, account, "count_tokens_passthrough", 1, s.tlsFPProfileService.ResolveTLSProfile(account))
	if err != nil {
		setOpsUpstreamError(c, 0, sanitizeUpstreamErrorMessage(err.Error()), "")
