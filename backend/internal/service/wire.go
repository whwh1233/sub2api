...
func ProvideBatchImageCleanupService(repo BatchImageRepository, accountRepo AccountRepository, cfg *config.Config) *BatchImageCleanupService {
	...
	startBackgroundWorker(svc.Start)
	return svc
}
...
func ProvideConcurrencyService(cache ConcurrencyCache, accountRepo AccountRepository, cfg *config.Config) *ConcurrencyService {
	...
	if cfg != nil {
		...
		startBackgroundWorker(func() { svc.StartSlotCleanupWorker(accountRepo, cfg.Gateway.Scheduling.SlotCleanupInterval) })
	}
	return svc
}
...
func ProvideOpsSystemLogSink(opsRepo OpsRepository) *OpsSystemLogSink {
	...
	startBackgroundWorker(sink.Start)
	logger.SetSink(sink)
	return sink
}
...
func ProvideAPIKeyAuthCacheInvalidator(apiKeyService *APIKeyService) APIKeyAuthCacheInvalidator {
	...
	startBackgroundWorker(func() { apiKeyService.StartAuthCacheInvalidationSubscriber(context.Background()) })
...
func ProvideOpsIngressRejectAggregator(opsRepo OpsRepository, opsService *OpsService) *OpsIngressRejectAggregator {
	...
	startBackgroundWorker(aggregator.Start)
