...
func TestGetDataDirForPlatformSkipsDockerPathOnWindows(t *testing.T) {
	t.Setenv("DATA_DIR", "")
	dockerDataDir := t.TempDir()

	if got := getDataDirForPlatform("windows", dockerDataDir); got != "." {
		t.Fatalf("getDataDirForPlatform(windows)=%q, want current directory", got)
	}
}

func TestGetDataDirForPlatformUsesDockerPathOnLinux(t *testing.T) {
	t.Setenv("DATA_DIR", "")
	dockerDataDir := t.TempDir()

	if got := getDataDirForPlatform("linux", dockerDataDir); got != dockerDataDir {
		t.Fatalf("getDataDirForPlatform(linux)=%q, want %q", got, dockerDataDir)
	}
}
